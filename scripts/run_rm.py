...
def main():
    ...
    config["model_builder"] = AutoModel.from_pretrained
    ...
    dataset = datasets.load_dataset("/home/ubuntu/hieu.nn/Lang/alignment-handbook/data/spin_data/iter0_2/ultrachat_200k_generated/0_50000", 
                                     split="train")
    dataset = dataset.rename_columns({"prompt_id": "id"})
    # copy id for saving, then remove
    ids = dataset["id"]
    dataset = dataset.remove_columns("id")
    def prepare_dialogue_from_tokenizer(example, tokenizer):
    ...
    for index in random.sample(range(len(dataset)), 3):
        ...
        logger.info(f"Chosen sample {index} of the raw training set:\n\n{dataset[index]['text_chosen']}")
        logger.info(f"Rejected sample {index} of the raw training set:\n\n{dataset[index]['text_rejected']}")   
    ...
    ############################
        ...
        for step, batch in enumerate(tqdm(dataloader, desc="RM batch steps")):
            ...
            if model_type == "Custom Classifier":
                ...
                # print(type(rewards_chosen))
                # print(rewards_chosen.keys())
    ...
    out_dataset = out_dataset.remove_columns(["prompt", "chosen", "rejected", "text_chosen", "text_rejected"])
    ...
    # Save out_dataset
    out_path = Path("/home/ubuntu/hieu.nn/Lang/alignment-handbook/data/spin_data/iter0_2/ultrachat_200k_generated/0_50000") / "reward_results"
    ...
    #     # Save out_dataset
    #     path = Path("/home/ubuntu/hieu.nn/Lang/alignment-handbook/data/spin_data/stablelm-2-1_6b-sft-full/ultrachat_200k_generated/1_61101")
    #     out_dataset.save_to_disk("out_dataset")
    # else:
